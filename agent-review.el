...
(defun agent-review-edit-comment-confirm ()
  ...
  (let* ((body (agent-review--edit-extract-body))
    ...
    (if remaining
      ...
      ;; All issues reviewed — submit
      (quit-window t)
      (pcase submit-mode
        ('review
         (when (y-or-n-p (format "Submit %d comment%s as %s review? "
                                 (length collected)
...
comment before submission.  When no issues are selected, submits
a review with only a body message.
  ...
  (let* ((issues (agent-review--get-marked-issues))
    ...
    (if issues
        (agent-review--edit-show-issue
         ...
         1 (length issues))
      ;; No issues — submit review with body only
      (when (y-or-n-p (format "Submit %s review with no inline comments? " event))
        (let ((url (agent-review--gh-submit-pr-review
