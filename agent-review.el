...
(defun agent-review--format-pr-review-comment (issue)
  "Format ISSUE plist as a GitHub PR review comment alist.
Returns alist with path, line, and body keys."
  (list (cons 'path (plist-get issue :file))
        (cons 'line (plist-get issue :line))
        (cons 'body (format "**[%s]** %s\n\n%s"
                            (upcase (plist-get issue :severity))
                            (plist-get issue :short-description)
                            (plist-get issue :diagnostic)))))

(cl-defun agent-review--gh-submit-pr-review (&key pr-url event issues)
...
ISSUES is a list of issue plists to post as review comments."
  (unless (executable-find "gh")
    (user-error "gh CLI not found.  Install it from https://cli.github.com"))
  (let* ((parsed (agent-review--parse-pr-url pr-url))
         (repo (car parsed))
         (number (cdr parsed))
         (comments (mapcar #'agent-review--format-pr-review-comment issues))
         (payload (json-encode
                   `((event . ,event)
                     (body . ,(format "Code review: %d issue%s found.\n\n---\n*Generated by agent-review.el*"
                                      (length issues)
                                      (if (= (length issues) 1) "" "s")))
...
(defun agent-review-submit-pr-review ()
  "Submit marked issues (or all issues) as a GitHub PR review.
Uses the gh CLI to post line-level review comments on the pull request."
  (interactive)
  ...
  (let* ((issues (agent-review--get-marked-issues))
    ...
    (unless issues
      (user-error "No issues to submit"))
    (when (y-or-n-p (format "Submit %d issue%s as %s review on %s? "
                            (length issues)
                            (if (= (length issues) 1) "" "s")
                            event
                            agent-review--pr-url))
      (let ((url (agent-review--gh-submit-pr-review
                  :pr-url agent-review--pr-url
                  :event event
                  :issues issues)))
        (kill-new url)
