...
(defun agent-review-blind-approve ()
  "Instantly approve a previously reviewed PR.
...
agent-review, then sends an APPROVE review to GitHub."
  ...
  (let ((urls (agent-review--blind-approve-load)))
    ...
    (let* ((choice (completing-read "Blind approve PR: " urls nil t)))
      (agent-review--parse-pr-url choice) ; validate URL
      (let ((url (agent-review--gh-submit-pr-review
                  :pr-url choice
                  :event "APPROVE"
                  :body "LGTM\n\n---\n*Generated by agent-review.el (blind-approve)*")))
        (kill-new url)
